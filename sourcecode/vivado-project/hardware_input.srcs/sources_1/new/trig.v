...
//当触发字(trig_word)和输入信号(input_data)相等时，且trig_mode为3，sys_valid有效时，word_trig才为true
...
always @ *
    case(edge_trig)
    //edge_trig 让 trigger 选择存取input_data第几位
...
always @ (posedge clk or negedge rst_n)
    ...
    //while trig_mode=3,auto=1; 即trig_mode为0、1、2、3时trig=1
    //trig=0时，trig_mode为错
